...
// Create the flat pixelated VIDEOTAPE monogram using shapes
const createFlatMonogram = (font) => {
  const text = "VIDEOTAPE";
  const group = new THREE.Group();
  
  // Get shapes from font for the text
  const textShape = font.generateShapes(text, 1.2); // Smaller size due to length
  ...
  const fillMaterial = new THREE.MeshBasicMaterial({ 
    color: 0xF08080, // Light coral color for videotape
    side: THREE.DoubleSide // Ensure visibility from both sides
  });
  
  const edgeMaterial = new THREE.LineBasicMaterial({ 
    color: 0x444444, // Dark gray outline
  ...
  // Scale the entire group - smaller due to longer text
...
// Animation loop with videotape-like motion
const animate = () => {
  ...
  if (monogram) {
    ...
    // VHS-like tracking effect - periodic jitter
    if (Math.sin(t * 5) > 0.95) {
      // Simulate vertical tracking problem
      monogram.position.y = (Math.random() - 0.5) * 0.1;
    } else {
      // Reset position
      monogram.position.y = 0;
    }
    
    // Slow, steady rotation
    monogram.rotation.y += 0.005;
    
    // VHS warping effect - gentle wave distortion
    monogram.rotation.x = Math.sin(t) * 0.15;
    
    // Occasional horizontal shift
    if (Math.random() < 0.01) {
      monogram.position.x = (Math.random() - 0.5) * 0.1;
      setTimeout(() => {
        monogram.position.x = 0;
      }, 100);
    }
  }
...
/* Add retro VHS scan lines for videotape effect */
